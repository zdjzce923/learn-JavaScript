...
// 在构造函数中直接声明方法会有些不完美，因为本质上每次创建新实例后也会新创建方法，相当于 new Function()，这意味着由相同构造函数创建的实例的方法的原型链和标识符都不同，新建一个实例就要多创建一个空对象保存方法。为什么不直接让这些实例共享方法呢？
// 可以在定义构造函数时将共有属性和方法定义在 prototype 上，之后创建的实例都会拥有这些属性和方法。
